...
# include this last name in our output. Last name "Astaire" is present only one 
# time with actor "Angelina Astaire", hence we would want this in our output list. 
USE sakila;

SELECT 
	first_name, last_name, COUNT(last_name) AS last_name_count
FROM 
	actor
GROUP BY
	last_name
...
# We would use the same logic as in the previous question but this time we 
...
SELECT 
	staff_id, COUNT(rental_id) AS rental_count
FROM 
	rental
GROUP BY
	staff_id;
...
SELECT 
	release_year, COUNT(film_id) AS films_released
FROM 
	film
GROUP BY
	release_year;
...
SELECT 
	rating, COUNT(film_id) AS film_ratings
FROM 
	film
GROUP BY
	rating;
...
SELECT 
	rating, ROUND(AVG(film.length), 2) AS avg_film_length
FROM 
	film
GROUP BY
	rating;
